...
def get_esai_user (user_name, user_id):
	...
	for friend_id in friends_id_list:
		status_count_of_friend = snaide.get_statuses_count_by (None, friend_id["guid"])
		if (status_count_of_friend < status_counts_of_user):
			friends_count_with_less_statuses = friends_count_with_less_statuses + 1

	status_factor = (friends_count_with_less_statuses/friends_count_of_user) * 100
	...
	for status in status_list:
		status_likes_count = status_likes_count + snaide.get_likes_on_single_status(status["guid"])
	average_likes_per_friend = status_likes_count/friends_count_of_user
	friends_count_with_less_likes = 0
	for friend_id in friends_id_list:
		friend_status_list = snaide.get_statuses_by(None , friend_id["guid"])
		friend_status_likes_count = 0
		for status in friend_status_list:
			friend_status_likes_count = friend_status_likes_count + snaide.get_likes_on_single_status(status["guid"])
	...
	likes_factor = (friends_count_with_less_likes/friends_count_of_user) * 100
...
print get_esai_user("Rahsaan Jacobi", None)